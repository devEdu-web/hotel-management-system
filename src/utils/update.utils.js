class GenerateObject {
    ...
    static addDescriptionObject(object) {
        ...
        if(object.bedrooms || object.bathrooms || object.livingRoom) {
            object.description = {}
            if(object.bedrooms) newObject.description.bedrooms = object.bedrooms
            if(object.bathrooms) newObject.description.bathrooms = object.bathrooms
            if(object.livingRoom) newObject.description.livingRoom = object.livingRoom
        }

        for(let [key, value] of Object.entries(newObject)) {
            if(key == 'bedrooms' || key == 'bathrooms' || key == 'livingRoom') {
                delete newObject[key]
            }
        }
